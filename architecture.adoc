...
It also receives events from `Kafka` like persisted/new message notification that are broadcasted to all suspended `SSE` connections.
...
A `Spring Boot` application which receives from `Kafka` new data to be serialized through `Couchbase` and aknowledges their consuption.
Once data are persisted, a notification is also sent through `Kafka` to broadcast this information to all registered consumers.

The API can be deployed as a set of instances where messages sent by `Kafka` will be balanced.
...
Extensions are playing the role of additional API.
Recommended technology for their implementation is `Spring Boot` but it's not mandatory.
However, an extension must follow the standards defined by `Reactivity` architecture:

* A micro service that registers to `ZooKeeper`
* Consumes data through `Kafka` connectors
* Persist data to `Couchbase` and send notification through `Kafka`
